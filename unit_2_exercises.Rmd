...
### Any length rolling average with the given year as the ENDPOINT:
...
### Any length rolling average with the given year as the MIDPOINT:

With the midpoint-version of the rolling average calculations, you will need to handle indexing differently if `num_years` is even vs. odd. So make sure you include an if statement in your function with an even vs. odd test. I usually use the modulus operator `%%` in R which returns the remainder of a division operation, so num `%%` 2 equals 0 for even numbers and returns 1 for odd numbers.

```{r, eval=FALSE}
# Set 5-yr avgs as default
# If midpoint=TRUE, 5 year average of 2005 is calculated from 2003-2007
# If midpoint=FALSE, 5 year average of 2005 is calculated from 2001-2005
calc_rolling_avg = function(data, num_years=5, midpoint=TRUE){ 
  data$roll_avg = NA # Initialize results column
  ...
  if(midpoint == TRUE){
  ...
  } else if (midpoint==FALSE) { # midpoint=FALSE so do an endpoint moving avg
    for (i in seq(dim(data)[1]))
    {
      if (i >= num_years & i <= (dim(data)[1])){
        data$roll_avg[i] = mean(data[c((i-(num_years-1)):i),'No_Smoothing'])
    ...
    print("midpoint param should be TRUE or FALSE") # could break out of function here
  }
...
# Use the calc_rolling_avg() function to calculate rolling averages
temp_anomaly_roll_avg = temp_anomaly
temp_anomaly_roll_avg$avg_5yr = calc_rolling_avg(temp_anomaly, midpoint=TRUE)
temp_anomaly_roll_avg$avg_10yr = calc_rolling_avg(temp_anomaly, num_years=10, midpoint=TRUE)
temp_anomaly_roll_avg$avg_20yr = calc_rolling_avg(temp_anomaly, num_years=20, midpoint=TRUE)
head(temp_anomaly_roll_avg)
